...
public class DriveIOComp extends DriveIO {
    ...
    private void loadFieldLayout() {
        try {
            ...
            leftFrontPhotonPoseEstimator = new PhotonPoseEstimator(aprilTagFieldLayout, leftFrontRobotToCam);
            leftBackPhotonPoseEstimator = new PhotonPoseEstimator(aprilTagFieldLayout, leftBackRobotToCam);
            rightFrontPhotonPoseEstimator = new PhotonPoseEstimator(aprilTagFieldLayout, rightFrontRobotToCam);
            rightBackPhotonPoseEstimator = new PhotonPoseEstimator(aprilTagFieldLayout, rightBackRobotToCam);
        } catch (Exception e) {
    ...
    private void updateOdometry() {
        ...
        try {
            ...
            frontLeftDrivePositions = frontLeftDrivePositionQueue.stream().mapToDouble(Double::doubleValue).toArray();
            frontRightDrivePositions = frontRightDrivePositionQueue.stream().mapToDouble(Double::doubleValue).toArray();
            backLeftDrivePositions = backLeftDrivePositionQueue.stream().mapToDouble(Double::doubleValue).toArray();
            backRightDrivePositions = backRightDrivePositionQueue.stream().mapToDouble(Double::doubleValue).toArray();
            ...
            yawPositions = yawPositionQueue.stream().map(Rotation2d::fromDegrees).toArray(Rotation2d[]::new);
            pitchPositions = pitchPositionQueue.stream().map(Rotation2d::fromDegrees).toArray(Rotation2d[]::new);
            rollPositions = rollPositionQueue.stream().map(Rotation2d::fromDegrees).toArray(Rotation2d[]::new);
    ...
    private void processPhotonResults(
        ...
        for (PhotonPipelineResult result : results) {
            Optional<PendingVisionObservation> observation = createPhotonObservation(logKey, estimator, result);
    ...
    private Optional<EstimatedRobotPose> estimatePhotonPose(PhotonPoseEstimator estimator, PhotonPipelineResult result) {
        ...
        Optional<Pose2d> referencePose = RobotState.getInstance().getEstimatedPoseAtTimestamp(result.getTimestampSeconds());
    ...
    private void updateLimelightObservation(List<PendingVisionObservation> pendingVisionObservations) {
        ...
        try {
            ...
            if (mt2.timestampSeconds <= 0.0) {
                Logger.recordOutput("Cameras/Limelight Pose", new Pose2d());
                return;
            }

            Rotation2d measurementTurretAngle =
                    turretAngleBuffer.getSample(mt2.timestampSeconds).orElse(currentTurretAngle);
            if (Math.abs(measurementTurretAngle.minus(currentTurretAngle).getDegrees())
                    > maxLimelightTurretMismatchDegrees) {
                Logger.recordOutput("Cameras/Limelight Pose", new Pose2d());
                return;
            }

            if (mt2.tagCount == 0 || mt2.avgTagDist > 4.5 || !poseNearField(mt2.pose)) {
                Logger.recordOutput("Cameras/Limelight Pose", new Pose2d());
                return;
            }

            Logger.recordOutput("Limelight dist to tag", mt2.avgTagDist);
            Logger.recordOutput("Cameras/Limelight Pose", mt2.pose);
            pendingVisionObservations.add(new PendingVisionObservation(
                    "Cameras/Limelight Pose",
                    mt2.pose,
                    mt2.timestampSeconds,
                    createVisionStdDevs(mt2.avgTagDist, mt2.tagCount, 2.0)));
        } catch (Exception e) {
    ...
    private boolean shouldAcceptOdometrySample(SwerveModulePosition[] wheelPositions, Rotation2d yawPosition) {
        ...
        for (int i = 0; i < wheelPositions.length; i++) {
            maxModuleDelta = Math.max(
                    maxModuleDelta,
                    Math.abs(wheelPositions[i].distanceMeters - lastAcceptedOdometryPositions[i].distanceMeters));
        }
    ...
    private void seedAcceptedOdometryState(SwerveModulePosition[] wheelPositions, Rotation2d yawPosition) {
        for (int i = 0; i < wheelPositions.length; i++) {
            lastAcceptedOdometryPositions[i] =
